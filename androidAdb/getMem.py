      ...
      for line in lines:
        ...
        if appname in line and 'pid' in line and 'kB' in line:
          ...
          if line_name in appname:
            mem_v = round(float(mem_v) / 1024, 2)  #单位用M表示
...
def time_control():
  ...
  while True:
    ...
    adb = "rm -f log_su/adb_mem_info.csv"
    d = os.system(adb)
    adb = "adb shell dumpsys meminfo > log_su/adb_mem_info.csv"
    d = os.system(adb)
    filename = "log_su/adb_mem_info.csv"
    get_mem()
    write_report()
